...
import individual_worm_helpers as ind_worm
...
for worm in wormblobs:
    ...
    xMin, xMax, yMin, yMax = ind_worm.findBoundingCoords(normalized_worm_img)
    xMin, xMax, yMin, yMax = ind_worm.updateBoundingCoords(xMin, xMax, yMin, yMax)

    #Get the skeleton of the worm
    worm_skeleton = ind_worm.findSkeleton(normalized_worm_img, xMin, xMax, yMin, yMax)
    plt.figure()
    ...
    endpoints = ind_worm.findEndpoints(non_binary_skel)

    #If the worm has two endpoints, process it as a worm
    if len(endpoints) == 2:
        #Find bounding coords of skeleton to crop into rectangle
        xMin, xMax, yMin, yMax = ind_worm.findBoundingCoords(worm_skeleton)
        #Crop into rectangle
        warped = ind_worm.cropWormIntoRectangle(worm_img, endpoints, xMin, xMax, yMin, yMax)
